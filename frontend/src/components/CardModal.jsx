...
const COVER_COLORS = [
  ...
  '#ef4444','#f59e0b','#06b6d4','#ec4899', null
];
...
export default function CardModal({ cardId, board, onClose, onCardUpdated, onCardDeleted }) {
  ...
  async function toggleDone() {
    const res = await cardsApi.update(cardId, { is_done: !card.is_done });
    setCard(prev => ({ ...prev, is_done: !prev.is_done }));
    onCardUpdated(res.data);
  }
  ...
  async function addChecklist(e) {
    ...
    setCard(prev => ({ ...prev, checklists: [...(prev.checklists || []), { ...res.data, items: [] }] }));
    setNewCheckTitle('');
  }
  ...
  if (loading) return (
    <div className="overlay" ref={overlayRef} onClick={handleOverlayClick}>
      <div className={styles.modal}><div className={styles.loadingState}><div className="spinner" /></div></div>
    </div>
  );
  if (!card) return null;
      ...
      <div className={styles.modal} id={`card-modal-${card.id}`}>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.main}>
              ...
              <h2 
                ...
                onClick={(e) => {
                  ...
                  while (target && target !== e.currentTarget) {
                    if (target.tagName === 'A') return; // Clicked a link entirely
                    target = target.parentNode;
                  }
                  setEditingTitle(true);
                }}
              >
                {renderTextWithLinks(card.title)}
              </h2>
            )}
            ...
            {/* Done button */}
            <button
              className={`${styles.doneModalBtn} ${card.is_done ? styles.doneModalBtnActive : ''}`}
              onClick={toggleDone}
              id={`modal-done-btn-${card.id}`}
            >
              {card.is_done ? '✓ Done — Click to undo' : '✓ Mark as Done'}
            </button>
            ...
            {card.members?.length > 0 && (
              <div className={styles.section}>
                ...
                <div className={styles.membersRow}>
                  {card.members.map(m => (
                    <div key={m.id} className={`avatar ${styles.memberAv}`}
            ...
            <div className={styles.section}>
              ...
              {editingDesc ? (
                <>
                  <textarea className={`input ${styles.descInput}`} value={descVal}
                    onChange={e => setDescVal(e.target.value)} placeholder="Add description…" autoFocus rows={4} />
                ...
                <div className={`${styles.descText} ${!card.description ? styles.descPlaceholder : ''} ${styles.markdownBody}`}
                  onClick={(e) => {
                    // Prevent linking clicks from opening edit mode
                    let target = e.target;
                    while (target && target !== e.currentTarget) {
                      if (target.tagName === 'A') return; // Don't trigger edit format if clicking a link
                      target = target.parentNode;
                    }
                    setEditingDesc(true);
                  }}>
                  {card.description ? (
                    ...
                    'Add a description… (supports Markdown text and links)'
                  )}
                </div>
              )}
            </div>
                ...
                <div key={cl.id} className={styles.section}>
                  <div className={styles.sectionHeader}>
                    <h4 className={styles.sectionLabel}>
                      ☑ {cl.title}
                      <span className={styles.clProgress}>{done}/{total}</span>
                    </h4>
                    <button className="btn btn-ghost btn-sm" onClick={() => deleteChecklist(cl.id)}>Delete</button>
                  </div>
                  ...
                  <div className={styles.checkItems}>
                    {cl.items.map(item => (
                      <div key={item.id} className={styles.checkItem}>
                        <input type="checkbox" id={`item-${item.id}`} checked={item.is_done}
                          onChange={() => toggleItem(cl.id, item.id, item.is_done)} className={styles.checkbox} />
                        <label htmlFor={`item-${item.id}`}
                          className={`${styles.checkLabel} ${item.is_done ? styles.checkLabelDone : ''}`}>
                          {item.text}
                        </label>
                        <button className="btn-icon" style={{ fontSize: '11px' }} onClick={() => deleteItem(cl.id, item.id)}>✕</button>
                      </div>
                    ))}
                    <div className={styles.addItemRow}>
                      <input className={`input ${styles.addItemInput}`} placeholder="Add an item…"
            ...
            <div className={styles.section}>
              ...
              <form onSubmit={submitComment} className={styles.commentForm}>
                <div className={`avatar ${styles.commentAvatar}`} style={{ background: '#0ea5e9' }}>A</div>
                <div className={styles.commentInputWrap}>
                  <textarea className={`input ${styles.commentInput}`} placeholder="Write a comment…"
                    value={newComment} onChange={e => setNewComment(e.target.value)} rows={2} />
                  {newComment.trim() && <button type="submit" className="btn btn-primary btn-sm mt-2">Save</button>}
                </div>
              </form>
              {(card.comments || []).map(c => (
                <div key={c.id} className={styles.comment}>
                  <div className={`avatar ${styles.commentAvatar}`} style={{ background: c.member_color || '#0ea5e9' }}>
                    {(c.member_name || 'U').split(' ').map(p => p[0]).join('').slice(0, 2)}
                  </div>
                  <div className={styles.commentContent}>
                    <div className={styles.commentMeta}>
                      <strong>{c.member_name || 'Unknown'}</strong>
                      <span className="text-muted text-xs">{c.created_at ? format(new Date(c.created_at), 'MMM d, h:mm a') : ''}</span>
                    </div>
          ...
          <div className={styles.sidebar}>
            ...
            <div className={styles.sidebarBlock}>
              ...
              <input type="date" className={`input ${styles.dateInput}`} value={dueDate}
                onChange={e => saveDueDate(e.target.value)} />
              {dueDate && <button className={`btn btn-ghost btn-sm ${styles.clearDue}`} onClick={() => saveDueDate('')}>Remove</button>}
            </div>
                ...
                {COVER_COLORS.map((color, i) =>
                  color ? (
                    <button key={color}
                      ...
                      style={{ background: color }} onClick={() => setCoverColor(color)} />
                  ) : (
                    <button key="none" className={styles.coverNone} onClick={() => setCoverColor(null)} title="Remove cover">✕</button>
                  )
                )}
              </div>
            </div>
            ...
            <div className={styles.sidebarBlock}>
              ...
              <form onSubmit={addChecklist} className={styles.addCheckForm}>
                <input className="input" placeholder="Title…" value={newCheckTitle}
            ...
            <div className={styles.sidebarBlock}>
              ...
              <button className="btn btn-ghost btn-sm w-full" style={{ marginBottom: 6 }} onClick={archiveCard}>📦 Archive</button>
              <button className="btn btn-danger btn-sm w-full" onClick={deleteCard}>🗑️ Delete</button>
            </div>
          </div>
        </div>
      </div>
